...
exports.viewAllCode = (req, res) => {
    ...
    mysqlConnection.query('select user_id from store_lock where id_lock = ? ', [id], (error1, rows1, fields) => {
        if (!error1) {
            mysqlConnection.query('select * from store_code where lock_id = ? AND user_id = ?', [id, rows1[0].user_id], (error, rows, fields) => {
                if (!error) {
                    res.json(rows);
                } else {
                    res.json(error);
...
exports.deleteCode = (req, res) => {
    var lock_id = req.body.id;
    mysqlConnection.query('DELETE from store_code where id = ?', [lock_id], (error, rows, fields) => {
        if (!error) {
...
exports.createShareCode = (req, res) => {
    ...
    mysqlConnection.query('select * from store_code where lock_id = ? AND code = ?', [lock_id, code], (error1, rows, fields) => {
        if (error1) return res.json({ status: "error", error: error1 });
        if (rows[0]) return res.json({ status: "exist", error: "code already exist" });
        else {
            //select id of user
            mysqlConnection.query('select id from auth_user where email = ?', [email_user], (error2, idrows, fields) => {
                if (error2) return res.json({ status: "error", error2: error2 });
                else {
                    mysqlConnection.query('select id from store_myuser where user_id=?', [idrows[0].id], (error3, rows3, fields) => {
                        if (error3) return res.json({ status: "error", error3: error3 });
                        else {
                            mysqlConnection.query('select * from store_code where lock_id = ? AND user_id = ?', [lock_id, rows3[0].id], (error4, nrows, fields) => {
                                if (error4) return res.json({ status: "error", error4: error4 });
                                if (nrows[0]) return res.json({ status: "exist", error: "user already have access" });
                                else {
                                    mysqlConnection.query('INSERT INTO store_code SET ?', { code: code, description: description, start_date: start_date, end_date: end_date, is_set: "0", lock_id: lock_id, user_id: rows3[0].id }, (error5, results) => {
                                        if (error5) return res.json({ status: "error", error5: error5 })
                                        else {
                                            console.log('code registered successfuly');
...
exports.viewAllShareCode = (req, res) => {
    var id = req.body.lock_id;
    mysqlConnection.query('select store_code.id, store_code.code, store_code.description,store_code.start_date,store_code.end_date,store_code.is_set, auth_user.username, auth_user.email from store_code,auth_user,store_myuser where store_code.lock_id=? AND store_code.user_id=store_myuser.id AND auth_user.id=store_myuser.user_id', [id], (error, rows, fields) => {
        if (!error) {
            res.json(rows);
        } else {
            console.log(error);
...
exports.getAllCodeAccessUser = (req, res) => {
    var id = req.body.user_id;
    mysqlConnection.query('select store_code.id,store_code.code, store_code.description,store_code.start_date,store_code.end_date,store_code.is_set, store_lock.lock_name, store_lock.lock_mac, store_lock.lock_data,store_lock.lock_percent from store_code,store_lock where store_code.user_id = ? AND store_code.lock_id=store_lock.id_lock', [id], (error, rows, fields) => {
        if (!error) {
            res.json(rows);
        } else {
            console.log(error);
...
exports.updatePasswordToLock = (req, res) => {
    var code = req.body.code;
    var user_id = req.body.user_id;
    var id = req.body.id;

    mysqlConnection.query('UPDATE store_code SET is_set=? WHERE id=? AND code=?', ['1', id, code], (error, results) => {
        if (error) return res.json({ status: "error" })
        else {
            console.log('code updated successfuly');
            return res.json({ status: "updated", success: "good code" });
