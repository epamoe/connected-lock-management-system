...
exports.createShareCard = (req, res) => {
    var lock_id = req.body.lock_id;
    var description = req.body.description;
    var user_id = req.body.user_id;
    var start = req.body.start_date;
    ...
    mysqlConnection.query('INSERT INTO store_card SET ?', { card: card, description: description, start_date: start_date, end_date: end_date, is_set: "0", lock_id: lock_id, user_id: user_id }, (error, results) => {
        if (error) return res.json({ status: "error" })
        else {
            console.log('card share successfuly');
            return res.json({ status: "success", success: "good card" });
        }
    });
...
exports.viewAllShareCard = (req, res) => {
    var id = req.body.lock_id;
    mysqlConnection.query('select store_card.id, store_card.card, store_card.description,store_card.start_date,store_card.end_date,store_card.is_set, auth_user.username, auth_user.email from store_card,auth_user where store_card.lock_id=? AND store_card.user_id=auth_user.id', [id], (error, rows, fields) => {
        if (!error) {
            res.json(rows);
        } else {
            console.log(error);
