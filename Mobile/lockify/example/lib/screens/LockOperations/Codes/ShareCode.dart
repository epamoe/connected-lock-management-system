...
import 'package:ttlock_flutter_example/models/ShareCodeModel.dart';
...
class ShareCode extends StatefulWidget {
  ...
  @override
  _ShareCodeState createState() =>
      _ShareCodeState(lockName, lockData, lockMac, idAdmin, idSerrure, percent);
}
...
class _ShareCodeState extends State<ShareCode> {
  ...
  int? idSerrure;
  int? percent;
  _ShareCodeState(String lockName, String lockData, String lockMac,
  ...
  static getCode(int id) async {
    var url = Uri.parse(Urls.url_get_all_share_code);
  ...
  Future attention(int id) => showDialog(
        context: context,
        builder: (context) => AlertDialog(
  ...
  Future<void> deletespecifiquecodeaccess(int id) async {
    EasyLoading.show(status: 'proceed to delete');
    var url = Uri.parse(Urls.url_delete_specifique_code);
    Map map = {
      "id": id.toString(),
                ...
                MaterialButton(
                  onPressed: () async {
                    if (_key.currentState!.validate()) {
                      ...
                      // print(endDates);
                      createCostumCode(
                          d, description, user, startDates, endDates);
                    }
                    Navigator.pop(context);
                  },
                  child: Text('validate'),
  ...
  Future<void> createCostumCode(
      int code, String a, String user, int startDate, int endDate) async {
    EasyLoading.show(status: 'proceed');
    String goodCode = code.toString();

    print(startDate);
    print(endDate);
    var url = Uri.parse(Urls.url_add_share_code);
    Map map = {
      "code": goodCode,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'exist') {
          print("----------------------error for add");
          EasyLoading.showInfo(data['error'], duration: Duration(seconds: 4));
        }
  ...
  getData() async {
    ...
    if (data != []) {
      ...
      for (Map i in data) {
        setState(() {
          _loading = false;
          _employees!.add(ShareCodeModel.fromJson(i));
          _codeList!.add(ShareCodeModel.fromJson(i));
        });
      }
    } else if (data == null) {
      setState(() {
  ...
  List<ShareCodeModel>? _employees;
  List<ShareCodeModel>? _codeList;
