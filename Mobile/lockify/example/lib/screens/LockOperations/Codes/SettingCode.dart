...
class SettingCostumCode extends StatefulWidget {
  SettingCostumCode({
  ...
  @override
  _SettingCostumCodeState createState() => _SettingCostumCodeState(lockName,
      lockData, idAdmin, idserrure,lockMac, percent);
}

class _SettingCostumCodeState extends State<SettingCostumCode> {
  String lockName = '';
  String lockData = '';
  String idAdmin = '';
  int? idserrure;
  ...
  _SettingCostumCodeState(String lockName,
      String lockData,
      String idAdmin,
      int idserrure,
      String lockMac,
  ...
  void modifyCostumCode(String name, int oldCode, int newCode, int startDate,
      int endDate) {
  ...
  Future<void> resetCostumCode() async {
    var url = Uri.parse(Urls.url_delete_all_costume_code);
    try {
      final response = await http.post(url, body: {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'deleted') {
          TTLock.resetPasscode(lockData, (lockData) async {
